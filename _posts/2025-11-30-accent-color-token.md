...
> **Correction — 1 August 2026:** The original version of this article described `accent-color-token` as a CSS feature. That was incorrect, so the erroneous article has been replaced.

CSS defines the [`accent-color`](https://www.w3.org/TR/css-ui-4/#widget-accent) property; it does not define a separate `accent-color-token` property, function, selector, or media query.
